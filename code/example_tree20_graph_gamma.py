...
for gamma in [0.1,1,10]:
    ...
    gr = graph_regularization(L, gamma=gamma)
    ...
    for i in [1,0]:
        ...
        plt.savefig('results/tree_smooth_delta_'+str(i)+'_gamma_'+str(gamma)+'.pdf',
                    bbox_inches='tight')
        #plt.savefig('results/tree_smooth_delta_'+str(i)+'_gamma_'+str(gamma)+'.eps'
        #            ,bbox_inches='tight')
        plt.show()
    ...
    sorted_centrality = {}
    sorted_keys = sorted(I_centrality, key=I_centrality.get)
    
    for w in sorted_keys:
        sorted_centrality[w] = I_centrality[w]
