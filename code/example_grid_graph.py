...
plt.savefig('results/grid.pdf', bbox_inches='tight')
#plt.savefig('results/grid.eps', bbox_inches='tight')
plt.show()
...
if use_EffG:
    I_EffG = EffG(G)
    names.append("EffG")
    centralities.append(I_EffG, method="shortest_path")
...
for i in range(n_names):
    ...
    plt.savefig('results/grid_'+names[i]+'.pdf', bbox_inches='tight')
    #plt.savefig('results/grid_'+names[i]+'.eps', bbox_inches='tight')
    plt.show()
    ...
    plt.xticks(range(len(G)), sorted_nodes, rotation=-90)
    plt.plot(range(len(G)), height, 'k', lw=1.2, zorder=1)
    plt.scatter(range(len(G)), height, s=100, c=height, cmap=cmap, zorder=2)
    plt.savefig('results/grid_'+names[i]+'_signature.pdf', bbox_inches='tight')
    #plt.savefig('results/grid_'+names[i]+'_signature.eps', bbox_inches='tight')
    plt.show()
