...
for i in range(n_names):    
    ...
    if disp_information:
        plt.title(names[i])
    nx.draw_networkx(G, pos, node_color=list(centralities[i].values()), node_size=20,
                      with_labels=False, cmap=cmap)
    #plt.axis('equal')
    plt.savefig('results/sensor_'+names[i]+'.pdf', bbox_inches='tight')
    #plt.savefig('results/sensor_'+names[i]+'.eps', bbox_inches='tight')
    plt.show()
    ...
    plt.scatter(range(len(G)), height, s=20, c=height, cmap=cmap, zorder=2)
    plt.savefig('results/sensor_'+names[i]+'_signature.pdf', bbox_inches='tight')
    #plt.savefig('results/sensor_'+names[i]+'_signature.eps', bbox_inches='tight')
    plt.show()
