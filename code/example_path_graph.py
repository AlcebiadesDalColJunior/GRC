...
pos_labels = dict()
for i, position in pos.items():
    pos_labels[i] = position + 0.01

plt.figure()
nx.draw_networkx(G, pos=pos, node_color="lightblue")
plt.savefig('results/path.pdf', bbox_inches='tight')
#plt.savefig('results/path.eps', bbox_inches='tight')
plt.show()
...
for i in range(n_names):
    ...
    plt.savefig('results/path_'+names[i]+'.pdf', bbox_inches='tight')
    #plt.savefig('results/path_'+names[i]+'.eps', bbox_inches='tight')
    plt.show()
    ...
    plt.savefig('results/path_'+names[i]+'_signature.pdf', bbox_inches='tight')
    #plt.savefig('results/path_'+names[i]+'_signature.eps', bbox_inches='tight')
    plt.show()
